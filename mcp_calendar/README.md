...
4.  **Environment Configuration:**
    *   This service no longer has its own `.env` file — it shares the
        monorepo's root `.env` (one directory up) with every other
        container. When run standalone (`python run_server.py` from inside
        `mcp_calendar/`), `load_dotenv()` walks up and finds that root
        `.env` automatically; inside Docker, `docker-compose.yml` passes
        the same vars through as container environment entries.
    *   Copy the monorepo's root `.env.example` to `.env` (if you haven't
        already) and fill in your **Client ID** and **Client Secret**
        ...
        ```
    *   Ensure `TOKEN_FILE_PATH` points to a location where the app can write the token file (the default `.gcp-saved-tokens.json`, written into `mcp_calendar/` when run standalone, is usually fine). This file is in `.gitignore`.
