...
def _with_local_tz_offset(dt_str: str) -> str:
    """Appends the machine's local UTC offset to a naive ISO datetime string.

    Google Calendar treats a dateTime with no 'Z'/offset and no accompanying
    timeZone field as UTC, which silently shifts events created from local
    (offset-less) times like '2026-07-25T13:01:00'.
    """
    if _TZ_OFFSET_RE.search(dt_str):
        return dt_str
    offset = datetime.now().astimezone().strftime("%z")  # e.g. "+0300"
    return f"{dt_str}{offset[:3]}:{offset[3:]}"
...
def create_mcp_server():
    ...
    async def create_event(calendar_id: str, summary: str, start_time: str, 
        ...
        try:
            data = {
                "summary": summary,
                "start": {"dateTime": _with_local_tz_offset(start_time)},
                "end": {"dateTime": _with_local_tz_offset(end_time)}
            }
            ...
            response = requests.post(
                ...
                # Interviews are booked on a shared account on the platform's
                # behalf — Google must never email attendees from it. The
                # attendee still ends up on the event; only the notification
                # is suppressed. Not exposed as a tool argument: this is
                # enforced unconditionally, not left to the model's judgment.
    ...
    async def update_event(calendar_id: str, event_id: str, summary: str = None, 
        ...
        try:
            ...
            if start_time:
                data["start"] = {"dateTime": _with_local_tz_offset(start_time)}
            if end_time:
                data["end"] = {"dateTime": _with_local_tz_offset(end_time)}
            if description:
                data["description"] = description
            if location:
                data["location"] = location
